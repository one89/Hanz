                    ...
                    <div class="card bg-transparent border-0 custom-border-radius-1">
                        <div class="card-body p-0 z-index-1">
                            <a href="{{ url('/blog/'.$post->slug) }}">
                                <img class="card-img-top border-radius-0 mb-2" src="{{ asset($post->image) }}" alt="Card Image">
                            </a>
                            <p class="text-uppercase text-color-default text-1 my-2">
                                <time pubdate datetime="{{ $post->date }}">{{ \Carbon\Carbon::parse($post->date)->format('d M Y') }}</time>
                            ...
                            <div class="card-body p-0">
                                <h4 class="card-title text-5 font-weight-semi-bold pb-1 mb-2">
                                    <a class="text-color-secondary text-color-hover-primary text-decoration-none text-4" href="{{ url('/blog/'.$post->slug) }}">{{ $post->title }}</a>
                                </h4>
                                <p class="card-text mb-2">{{ Str::limit(strip_tags($post->content), 120) }}</p>
                                <a href="{{ url('/blog/'.$post->slug) }}" class="text-decoration-none custom-link-hover-effects">
